...
export const AccountsProvider = ({ children }) => {
  ...
  // Obtener EAs asociados a una cuenta
  const getAccountEAs = useCallback(
    (accountIdentifier) => {
      // Convertir accountIdentifier a string para comparación consistente
      const accountIdentifierStr = accountIdentifier.toString();
      console.log("Buscando EAs para la cuenta:", accountIdentifierStr);

      const filteredEAs = localEAs.filter((ea) => {
        // Primero intentar coincidir con accountId (compatibilidad con versiones anteriores)
        if (ea.accountId) {
          const eaAccountId = ea.accountId.toString();
          if (eaAccountId === accountIdentifierStr) {
            return true;
          }
        }
        
        // Luego intentar coincidir con accountNumber (nuevo método)
        if (ea.accountNumber) {
          const eaAccountNumber = ea.accountNumber.toString();
          if (eaAccountNumber === accountIdentifierStr) {
            return true;
          }
        }
